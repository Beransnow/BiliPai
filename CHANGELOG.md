...
- 全局统一顶部标签、筛选、排序及模式切换控件为 Miuix 原生分段样式，移除页面级浮动 Dock，避免内容背景变化导致文字对比度不稳定。
- 液态玻璃开关不再影响分段控件的基础样式；搜索框、底部导航及评论操作栏继续按开关使用对应材质。
- 优化 Miuix 深色页面的背景层级、选中态和未选中态颜色，消除黑色与灰色区域的视觉断层。

### 首页滚动交互

- 顶部栏折叠仅在列表离开顶部后生效，回到顶部时优先展开顶部栏，再交由下拉刷新处理剩余手势。
- 增加拖拽结束后的顶部栏状态收敛，避免停留在半折叠状态。

### 视频详情与返回转场

- 视频详情返回媒体层仅在过渡阶段保留封面圆角，抵达详情播放器后恢复全屏矩形显示。
- 平板及大屏设备的全屏返回转场统一使用方形宿主裁剪，避免残留卡片圆角。
