...
[添加] 新增推荐流过滤插件
[添加] 番剧页以原生 Compose 重建并接入 PiliPlus 数据接口，番剧与评论区加载骨架屏
[添加] 收藏夹完整分类管理，收藏、历史、稍后再看、个人列表管理体验对齐
...
[修复] 底栏指示器点按/拖拽动画与选中态同步，顶栏标签滚动恢复与滑动路由
[修复] 竖屏长按倍速提示被状态栏遮挡，评论楼中楼不再盖住播放器上方阴影
[修复] 关闭过渡动画后首页与播放页改用安卓原生横滑返回，卡片形变与返回交接
[修复] 个人列表边缘手势冲突，重复播放后封面遮罩，首页信息流滚动热路径性能
