...
[适配] Android 17 新增本地网络运行时权限：DLNA、SSDP 与本地代理仅在授权后访问局域网，拒绝或撤销权限后立即停止相关发现
[更改] 投屏设备页补充本地网络权限说明、授权入口和系统设置跳转；未授权时 Google Cast 仍可独立使用
[适配] 后台音频服务迁移到 MediaSessionService，继续复用现有媒体会话、前台通知、蓝牙/耳机按键和画中画控制链路
[适配] 手机继续按播放器状态请求横竖屏；平板、折叠屏及大窗体释放物理方向锁定，横屏播放改由应用内布局管理
[添加] 接入 Android 17 播放接力，支持普通视频、番剧和听视频模式携带播放位置继续播放，并提供网页回退地址
[添加] 接入 Android 17 系统 Profiling 触发与内存限制退出诊断；诊断文件保存在应用私有目录，可随日志导出并受数量、总容量限制
[更改] 关闭崩溃诊断时同步注销 Profiling 触发并删除本地诊断文件，备份规则也排除这些临时产物
[添加] 新增 Android 17 验证清单与原生兼容检查脚本，覆盖新 MessageQueue、16 KB 页面/ELF 对齐、本地网络、后台音频、大屏、接力、内存限制和音频加固场景

### 登录与账号
[修复] 恢复短信、密码等非二维码登录链路，二维码登录保持原有流程
[修复] Geetest 在页面容器创建完成后再初始化，避免验证码界面尚未挂载就执行导致验证失败
[修复] Passport 返回风险验证时可回放原登录请求，并在验证未通过时按策略重新获取挑战而不是直接中断
[修复] 短信和密码登录统一使用 Android HD 客户端身份、请求参数与设备标识，减少接口身份不一致
[修复] 短信国家区号与验证码参数按客户端接口规范发送，Cookie 导入不再覆盖关键登录字段
[更改] 登录设备身份改为持久化复用，移除不稳定的网页短信兜底，避免切换请求路径后登录态丢失

### 护眼与插件反馈
[添加] 插件启用、关闭或实际生效时显示应用内提示卡，减少功能状态不明确的问题
[添加] 弹幕增强、护眼等内置插件接入统一效果提示，并在插件页展示当前运行状态
[更改] 夜间护眼改为按设定时间渐进启用和退出的柔和暖色遮罩，降低颜色突变
[更改] 视频播放期间自动减弱护眼强度，离开播放后平滑恢复，避免遮罩影响画面辨识
[更改] 护眼设置补充时间段、强度和实时状态说明，配置变化可立即反映到覆盖层

### 液态玻璃与紧凑控件
[更改] 独占双选分段控件统一复用紧凑液态 Dock，覆盖番剧长评排序、动态评论排序、动态发布/投票、首页热门子分类与「今日看点」模式切换
[更改] UP 主空间与搜索结果标签统一复用紧凑液态 Dock，减少各页面重复的指示器和采样实现
[更改] 紧凑 Dock 的高光、折射、阴影和按压扩散按实际控件高度缩放，避免短控件出现中线发黑或折射过强
...
[修复] 从搜索结果进入详情再返回时恢复原搜索词、结果列表、筛选条件与滚动位置
[修复] 搜索标签与筛选切换不再意外清空已加载结果，页面重建后仍回到离开前的内容状态

### 视频、竖屏预览与弹幕
[添加] 竖屏视频拖动进度时显示缩略图预览，预览气泡跟随当前进度位置
[修复] 竖屏进度预览按视频横竖比例选择展示尺寸，避免缩略图被拉伸、裁切或遮住进度条
[修复] 竖屏视频切换时隔离预览状态，避免上一条视频的缩略图、时间或拖动状态残留到下一条
[修复] 直播列表与竖屏视频切换、返回时保留正确的播放和评论上下文，减少内容与当前视频错位
[修复] 弹幕加载、播放同步、进度跳转和渲染时序，降低播放中弹幕停滞、重复或位置错乱的概率

### 动态评论与首页
[修复] 动态详情评论输入栏固定在滚动内容之外，加载更多评论时不再反复闪动
[修复] 动态评论分页加载期间保持输入区和已有评论稳定，避免状态切换导致整页内容重组
[修复] 首页「今日看点」卡片折叠内容时的异常提前返回，推荐列表可以继续正常绘制
