...
[更改] 版本号 0.2.3-beta.33，versionCode 356
[更改] 正式包 BiliPai-0.2.3-beta.33.apk，Dev 包 BiliPai-0.2.3-beta.33-dev.apk
[更改] 基准为 beta.32 版本整理提交（e8c5ec7d，versionCode 354）

### 底栏 Dock 与液态指示器
[优化] 指示器按压缩放由 tween 改为 spring(0.6/250) 弹簧，横向与纵向形变拆分为两条独立动画，不再与拖拽进度取最大值合并
[修复] 释放指示器不再等待放大动画播放完毕，位置稳定后立即开始收缩，松手后的椭圆拉伸不再残留
[更改] Dock 几何统一为 64dp 外壳、56dp 静止指示器（上下各内缩 4dp）与 78dp 按压膨胀
[更改] 指示器透镜折射回到 10dp/14dp 基准，去掉额外的 1.35/1.40 放大补偿
[修复] 分区侧栏指示器与主页 Dock 共用同一套按压缩放与选中色

### 评论与图片回复
[添加] 评论输入框恢复图片上传入口，楼中楼的嵌套回复同样支持发送图片
[修复] 图片发布策略对齐 PiliPlus，不再依据编辑器图标状态禁用发图
[修复] 发送回复前提前捕获回复目标，修复发送瞬间输入框关闭导致回复目标丢失、误发成根评论的问题
[优化] 回复成功后自动关闭输入框并清理 @ 搜索状态，回复内容自动带上“回复 @昵称”前缀

### 翻页与 Tab 动效
[优化] 新增统一的 Pager 逐页滚动实现，底栏、首页顶部分类、收藏夹分栏与直播互动面板共用，内容与指示器不再因远距离预跳而硬切
[修复] 长距离拖动顶部 Tab 指示器时轨道同步滚动，指示器始终保持完整可见
[修复] 收藏夹分栏首次进入保持直接定位，后续切换才走逐页动画，避免多余的滚动动效

### 图片选择与系统适配
[优化] 图片选择优先唤起系统相册（ACTION_GET_CONTENT），无可用相册时回退 AndroidX 取图器；评论、动态发布、私信、头像、开屏壁纸与液态玻璃预览统一使用该入口
[修复] 播放器始终保留用户请求的物理屏幕方向，大屏与平板不再因平台限制退回 UNSPECIFIED，Android 16 通过兼容属性、Android 17 通过应用宽高比设置可正常生效

v0.2.3-beta.33(f3808160)
