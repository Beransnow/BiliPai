...
[更改] 版本号 0.2.3-beta.33，versionCode 358
[更改] 正式包 BiliPai-0.2.3-beta.33.apk，Dev 包 BiliPai-0.2.3-beta.33-dev.apk
[更改] 基准为 beta.33 版本整理提交（eec1a4c74，versionCode 357）

### 界面与主题
[优化] 全局统一首页、动态、搜索、详情、空间、直播、音频及插件中的顶部标签、筛选、排序和模式切换控件为 Miuix 原生分段样式，移除页面级浮动 Dock
[修复] 液态玻璃开关切换时分段控件不再改变基础样式，避免内容背景变化导致文字对比度不稳定
[优化] 调整 Miuix 深色页面背景层级、选中态和未选中态颜色，消除黑色与灰色区域的视觉断层

### 首页滚动交互
[修复] 顶部栏折叠仅在列表离开顶部后生效，回到顶部时优先展开顶部栏，再交由下拉刷新处理剩余手势
[优化] 增加拖拽结束后的顶部栏状态收敛，避免停留在半折叠状态

### 视频详情与返回转场
[修复] 视频详情返回媒体层仅在过渡阶段保留封面圆角，抵达详情播放器后恢复全屏矩形显示
[修复] 平板及大屏设备的全屏返回转场统一使用方形宿主裁剪，避免残留卡片圆角
