...
[修复] 切集/相关推荐后弹幕自动上屏，弹幕引擎按播放意图启动
[修复] 恢复评论上滑缩小播放器并修复回竖屏铺满
[修复] 退出后保持小窗播放与音量
[修复] 打开视频详情页期间抑制父级列表刷新
[更改] 相关推荐卡片排版紧凑化，标题/作者/播放量/弹幕间距收紧并对齐首页 token 体系
[添加] 动态图片查看原图支持全分辨率解码（上限 8192px）
[修复] 顶部「追番」标签点击直接进入番剧独立页
[更改] 顶部液态玻璃默认关闭；新增「下滑自动隐藏顶部栏」开关（默认开启）
[修复] 首页顶栏标签去除玻璃边缘，各主题保持可读
[更改] 顶栏控件与液态玻璃解耦
[更改] 首页卡片「已关注」紧跟 UP 名，时长移入统计行
[更改] 竖屏视频流推荐改用 Story 接口提升多样性
[更改] 默认顶栏标签改为 5 项（移除分区），Story 页与底栏分页同步
[更改] 搜索图标与清除按钮换位
[添加] 设置新增「列表条目样式」选项：自定义条目 / 原生组件
[修复] 开关浅色 thumb 不再变黑，开关与条目振动跟随全局开关
[更改] 设置单一入口分类点击直接进入下一级，减少菜单层级
[更改] UP 主徽章与头像默认关闭并全局生效
[修复] 设置界面主题图标恢复 ColorLens
[适配] MD3 下底栏可用项目图标跟随主题色
[更改] 全 app primary 填充按钮浅色主题统一用 primaryContainer
[适配] 对齐 MD3 官方 ColorScheme 全角色与 surface 层级
[更改] 全局图标样式两套：主题色容器 / MD3 官方推荐
[更改] 移除 iOS/Cupertino 视觉路径，主题收敛为 Miuix / Material 两值化策略
[修复] 投票/打分弹幕与 BAS 解析器缺陷
[修复] WebDAV 恢复/备份卡在创建远端目录 —— 改为探测优先
[修复] DLNA 发现兼容 Android XML 解析器与 SSDP 1900 端口
[添加] DLNA 远程播放控制与路由缓存
v0.2.1(72d7e97b)
