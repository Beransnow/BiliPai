...
- 本版覆盖默认性能设置、版本规范、收藏夹风控、听视频 UI 可读性、收藏预测返回对齐首页。
...
- **沉浸状态栏**保持默认关闭（未改）。
- **播放器洞察**默认关闭：首播不再默认叠加编码/码率/掉帧浮层；需要可在播放设置里选智能/始终显示。

#### 听视频

- 右上角「更多」操作表、待播清单、手动匹配歌词、歌词设置 sheet 与封面色板解耦。
- 弹层统一使用主题 `surface` / `onSurface` / `onSurfaceVariant`，修复 MD3 下浅色底+浅色字、深色底+深色字几乎不可读的问题。
...
- 订阅/收藏合集详情预测返回与普通 pop 改为 `NO_OP_SHARED_ELEMENT`，与首页 sharedBounds 整卡 morph 一致（跟手飞回列表源卡）。
