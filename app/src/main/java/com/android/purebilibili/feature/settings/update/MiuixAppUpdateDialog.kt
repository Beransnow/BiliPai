...
internal fun MiuixAppUpdateDialog(
    ...
    WindowDialog(
        show = true,
        title = if (state.showReleaseNotesOnly) "更新日志" else "发现新版本",
        summary = "版本 ${state.update.latestVersion}",
...
private fun MiuixUpdateActions(
    ...
    Row(
        ...
        TextButton(
            text = if (state.showReleaseNotesOnly) {
                "关闭"
            } else {
                appUpdatePrimaryLabel(state.downloadState, state.hasAsset)
            },
            onClick = if (state.showReleaseNotesOnly) actions.onDismissRequest else actions.onPrimaryAction,
            modifier = Modifier.weight(1f),
            enabled = state.showReleaseNotesOnly || !state.downloadState.isActiveDownload(),
