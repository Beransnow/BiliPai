...
fun LiveDanmakuOverlay(
    ...
    AndroidView(
        ...
        update = { view ->
            val textSize = 34f * danmakuSettings.fontScale.coerceIn(0.3f, 2f)
            view.engine.updateConfig(
                DanmakuRenderConfig(
                    alpha = (danmakuSettings.opacity.coerceIn(0f, 1f) * 255).toInt(),
                    textSizePx = textSize,
                    strokeWidthPx = danmakuSettings.strokeWidth.coerceAtLeast(0f),
                    ...
                    lineHeightPx = textSize * danmakuSettings.lineHeight.coerceAtLeast(1f),
                    pinnedDurationMs = (danmakuSettings.staticDurationSeconds.coerceAtLeast(1f) * 1_000L).toLong()
                )
            )
        }
    )

    LaunchedEffect(engine, isStarted, danmakuSettings.fontScale) {
        while (isActive) {
    ...
    LaunchedEffect(danmakuFlow, danmakuSettings) {
        ...
        danmakuFlow.collect { item ->
            ...
            val currentTime = SystemClock.elapsedRealtime() - startTime
            val textSize = 34f * danmakuSettings.fontScale.coerceIn(0.3f, 2f)
            val renderItem = createLiveDanmakuItem(item, currentTime, context, currentEngine, textSize)
