...
class ProfileShapeStructureTest {
    ...
    fun `profile surfaces use semantic theme shapes`() {
        ...
        assertTrue(screenSource.contains(".miuixLayerBackdrop(profileTabBackdrop)"))
        assertTrue(screenSource.contains(".biliPaiFloatingDockShell("))
        assertTrue(screenSource.contains("enabled = liquidGlassEnabled"))
        assertTrue(screenSource.contains("drawLens = false"))
