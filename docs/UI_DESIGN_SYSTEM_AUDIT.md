...
| 阶段 3：普通 feature | 30% | 卡片/Surface 首批已完成；`AppTextField/AppSearchField` 输入语义与 renderer 边界已覆盖普通 feature 首批 |
...
| 调用内部换肤 IOS* 的 feature | 阶段 3 当前 27（基线 42；旧报告口径 21） | 空间页搜索已切到中性入口；普通 feature 的 Dialog/Sheet 等仍待迁移 |
